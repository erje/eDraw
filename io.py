...
def save_ely(project, filename):
    ...
    uhr = "{day}.{month}.{year} {hour}:{minute}:{seconds}".format(day = ltime.tm_mday, month = ltime.tm_mon, year = ltime.tm_year, hour = ltime.tm_hour, minute = ltime.tm_min, seconds = ltime.tm_sec)

    etree.SubElement(ELAYOUT, "VERSION", {"created" : uhr, "modified" : uhr, "number" : "1.0"})
    etree.SubElement(ELAYOUT, "GRID", {"horizontal" : str(project.grid_horizontal), "show" : str(project.grid_show).lower(), "snap_to" : str(project.grid_snap_to).lower(), "vertical" : str(project.grid_vertical)})

    LAYER_LIST = etree.SubElement(ELAYOUT, "LAYER_LIST")
    for structure in project:
        for layer in structure:
            etree.SubElement( LAYER_LIST, "LAYER", {"fill_color" : layer.fill_color, "fill_opacity" : str(layer.fill_opacity), "hidden" : str(layer.hidden).lower(), "locked" : str(layer.locked).lower(), "name" : layer.name} )

    STRUCTURE_LIST = etree.SubElement(ELAYOUT, "STRUCTURE_LIST")
    for structure in project:
        STRUCTURE = etree.SubElement( STRUCTURE_LIST, "STRUCTURE", {"locked" : str(structure.locked).lower(), "name" : structure.name } )
        etree.SubElement(STRUCTURE, "VERSION", {"created" : uhr, "modified" : uhr, "number" : "1.0"})
        etree.SubElement(STRUCTURE, "INSTANCE_LIST")
        
        for layer in structure:
            LAYER_REFERENCE = etree.SubElement(STRUCTURE, "LAYER_REFERENCE", {"frame_cx" : str(layer.frame_cx), "frame_cy" : str(layer.frame_cy), "frame_size" : str(layer.frame_size), "ref" : layer.name})
            for shape in layer:
                LAYER_REFERENCE.append(etree.Element(shape.tag, shape.get_attrib()))
...
def save_svg(project, filename):
    try:
        ...
        raise ImportError("No module found for svg output. Please install module svgwrite.")
        ...
        for layer in structure:
            ...
            for shape in layer:
                if shape.__class__.__name__ == "Rect":
                    svg_layer.add( dwg.rect(insert=(shape.x, shape.y), size=(shape.width, shape.height), fill = layer.fill_color, fill_opacity = layer.fill_opacity) )
                elif shape.__class__.__name__ == "Polygon":
                    svg_layer.add( dwg.polygon(points = shape.points, fill = layer.fill_color, fill_opacity = layer.fill_opacity) )
                elif shape.__class__.__name__ == "Lines":
                    svg_layer.add( dwg.polyline(points = shape.points, stroke = layer.fill_color, stroke_opacity = layer.fill_opacity) )
